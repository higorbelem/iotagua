...
void loop() {    
    ...
    if(SNA > 920 ){
      ...
      // Salva na variavél atual, qual é o nivel lido no SNA
         if(SNA >= 920) atual = 1;
          if(SNA >= 918 && SNA <= 919) atual = 2;  // Valores obtidos através de teste
           if(SNA >= 915 && SNA <= 917) atual = 3;
            if(SNA >= 909 && SNA <= 914) atual = 4;
             if(SNA >= 899 && SNA <= 908) atual = 5;
              if(SNA >= 888 && SNA <= 898) atual = 6;
               if(SNA >= 871 && SNA <= 887) atual = 7;
...
// -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------               
   ...
   // SFA2 = contaPulso2 / 5.5; //Converte para L/min
    ...
    if (client.connect(server,80)) {
        ...
        client.print(String("POST /dweet/for/iotaguassv2?SensorFluxo1=")  + String(SFA) + "&SensorFluxo2=" + String(SFA2) + "&SensorNivel=" + String(atual) + "&Rele=" + String(rele) +  " HTTP/1.1\r\n" + "Host: " + server + "\r\n" + "Connection: close\r\n\r\n");
